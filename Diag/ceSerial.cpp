...
ceSerial::ceSerial(std::string Device, int64_t BaudRate, int64_t DataSize, uint8_t ParityType, float NStopBits) : stdbaud(true)
{
#ifdef CE_WINDOWS
...
void ceSerial::SetDataSize(int64_t nbits)
{
	if ((nbits < 5) || (nbits > 8))
		nbits = 8;
	dsize = nbits;
...
int64_t ceSerial::GetDataSize()
{
	return dsize;
}

void ceSerial::SetParity(uint8_t p)
{
	if ((p != 'N') && (p != 'E') && (p != 'O'))
	{
#ifdef CE_WINDOWS
...
// STRING ONLY!!!!
bool ceSerial::WriteString(uint8_t *data)
{
	if (!IsOpened())
	{
		return false;
...
// Binary
bool ceSerial::Write(uint8_t *data, uint64_t n)
{
	if (!IsOpened())
	{
		return false;
...
bool ceSerial::WriteByte(uint8_t ch)
{
	return Write(&ch, 1);
}

uint8_t ceSerial::ReadChar(bool &success)
{
	success = false;
	if (!IsOpened())
	{
